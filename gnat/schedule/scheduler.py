...
:class:`~gnat.schedule.job.FeedJob` instances.
...
2. **Persist job history** to a :class:`~gnat.context.store.FlatFileStore`
   or :class:`~gnat.context.store.WorkspaceStore` by attaching an
...
class FeedScheduler:
    """
    Threading engine for :class:`~gnat.schedule.job.FeedJob` instances.
    ...
    def statuses(self) -> list[dict]:
        ...
        Each dict is the output of :meth:`~gnat.schedule.job.FeedJob.status_dict`.
        """
    ...
    def to_celery_tasks(self, celery_app: Any) -> dict[str, Any]:
        ...
        Each job becomes a Celery task named ``gnat.feed.<job_id>``.
    ...
    def to_cron_lines(
        ...
        script_path : str
            Path to the runner script.  Default ``"gnat_run_job.py"``.
