...
class SplunkConfig:
    ...
    def _validate(self) -> None:
        ...
        if self.scheme not in ("http", "https"):
            raise SplunkConfigError(f"Invalid scheme '{self.scheme}'. Must be 'http' or 'https'.")
...
def load_splunk_config(
    ...
    if not config.has_section(section):
        raise SplunkConfigError(f"Configuration section '[{section}]' not found in gnat.ini.")
