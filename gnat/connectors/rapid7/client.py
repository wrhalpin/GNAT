...
class Rapid7Client(BaseClient, ConnectorMixin):
    ...
    def health_check(self) -> bool:
        if self._product == "insightvm":
            resp = self.get("/vm/v4/integration/vulnerabilities",
                            params={"size": 1})
        else:
            resp = self.get("/public/v2/iocs/ioc-by-value",
                            params={"iocValue": "google.com"})
    ...
    def _ioc_to_stix(self, native: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sectors = [t for t in tags if t not in
                   ("malware", "phishing", "botnet", "c2", "ransomware")]
