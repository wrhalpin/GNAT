"""
ctm_sak.codegen.openapi_generator
===================================
...
Usage (CLI)::
    ...
    python -m ctm_sak.codegen.openapi_generator \\
...
def generate_connector(
    ...
    out_dir: str = "./ctm_sak/connectors",
    test_dir: str = "./tests/unit/connectors",
    overwrite: bool = False,
) -> None:
    """
    Generate a CTM-SAK connector package from an OpenAPI specification.
    ...
    print()
    print("Next steps:")
    print(f"  1. Register '{name}' in ctm_sak/clients/__init__.py CLIENT_REGISTRY")
...
def _render_client(
    ...
    return textwrap.dedent(f'''\
        """
        ctm_sak.connectors.{name}.client
        {"=" * (len(name) + 30)}
...
def _render_init(name: str, class_name: str) -> str:
    return textwrap.dedent(f'''\
        """ctm_sak.connectors.{name} — auto-generated connector."""
        from ctm_sak.connectors.{name}.client import {class_name}
        __all__ = ["{class_name}"]
        ''')
...
def _render_tests(name: str, class_name: str) -> str:
    return textwrap.dedent(f'''\
        """
        Unit tests for ctm_sak.connectors.{name}
        Auto-generated by ctm_sak.codegen.openapi_generator
        """
...
def _main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate a CTM-SAK connector from an OpenAPI spec."
    )
    ...
    parser.add_argument("--out-dir",   default="./ctm_sak/connectors",
