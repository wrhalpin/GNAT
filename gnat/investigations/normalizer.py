...
def _xsoar_incident(platform: str, raw: dict[str, Any]) -> EvidenceNode:
    ...
    hostnames = [
        str(custom[k])
        for k in ("src_hostname", "dest_hostname", "hostname")
        if k in custom and custom[k]
    ]
    usernames = [
        str(custom[k])
        for k in ("src_user", "dest_user", "username", "account_id")
        if k in custom and custom[k]
    ]
    ...
    campaign_labels = [
        ...
        and lbl.get("type", "").lower() in ("campaign", "actor", "malware")
        and lbl.get("value")
    ]
...
def _gm_incident(platform: str, raw: dict[str, Any]) -> EvidenceNode:
    ...
    blob = f"{title} {desc}"
    ticket_refs = _extract_tickets(blob)
    campaign_labels = [t for t in data.get("tags", []) if isinstance(t, str) and len(t) > 2]
...
def normalize(
    ...
    if fn is None and record_type in ("observable", "indicator"):
        fn = _DISPATCH.get((platform.lower(), "observable")) or _DISPATCH.get(
            (platform.lower(), "indicator")
        )
    if fn is None:
        return None
