...
def test_broker_stores_and_resolves_secret() -> None:
    ...
    policy = SecretPolicyEngine([PolicyRule(path_prefix="dev/", actions=["store", "resolve"], allowed_callers=["runtime", "ci"], overwrite=True)])
    ...
    broker.store(StoreSecretRequest(ref=ref, value="super-secret", tags={"connector": "alienvault"}, allow_overwrite=True), caller="ci")
...
def test_broker_blocks_policy_violation() -> None:
    provider = MemorySecretProvider()
    policy = SecretPolicyEngine([PolicyRule(path_prefix="dev/", actions=["resolve"], allowed_callers=["runtime"])])
...
def test_connector_config_resolver_replaces_secret_ref() -> None:
    ...
    broker.store(StoreSecretRequest(ref=ref, value="resolved-token", allow_overwrite=True), caller="ci")
...
def test_unsafe_pattern_detector_flags_inline_credentials() -> None:
    findings = UnsafePatternDetector().inspect_connector_config({"credentials": {"api_key": "plain-text-value", "token": {"value": "still-plain-text"}}})
