...
import pytest
from gnat.agents.workflow import Workflow, WorkflowContext, WorkflowStep, WorkflowResult
...
def test_workflow_run_success():
    ...
    def step1(ctx): log.append("s1"); ctx.shared["flag"] = True
    def step2(ctx): log.append("s2"); return 42
...
def test_workflow_step_failure_aborts():
    ...
    def bad_step(ctx): raise ValueError("boom")
    def after(ctx):    log.append("after")
...
def test_workflow_on_failure_routing():
    ...
    def bad(ctx):     raise RuntimeError("oops")
    def fallback(ctx): log.append("fallback")
...
def test_workflow_on_success_routing():
    ...
    def s1(ctx): log.append("s1")
    def s2(ctx): log.append("s2")
    def skip(ctx): log.append("skip")
