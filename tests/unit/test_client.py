...
class TestGNATClient:
    ...
    @pytest.mark.parametrize(
        "target", ["threatq", "proofpoint", "netskope", "crowdstrike", "xsoar", "recordedfuture"]
    )
...
class TestBaseClient:
    ...
    def test_get_calls_pool_manager(self):
        c = self._make_client()
        c._http.request = MagicMock(return_value=self._mock_response(200, {"ok": True}))
        result = c.get("/test")
    ...
    def test_post_serialises_json(self):
        c = self._make_client()
        c._http.request = MagicMock(return_value=self._mock_response(200, {"created": True}))
    ...
    def test_4xx_raises_sak_client_error(self):
        c = self._make_client()
        c._http.request = MagicMock(return_value=self._mock_response(404, {"error": "not found"}))
    ...
    def test_query_params_appended_to_url(self):
        c = self._make_client()
        c._http.request = MagicMock(return_value=self._mock_response(200, {}))
